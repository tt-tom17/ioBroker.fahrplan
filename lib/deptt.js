...
import fDepTTDep from "./depttdep.js";
...
const sDepartureHTMLHeader = {
	...
	"zh-cn":
		'<tr><th align="center">时间</th><th align="left">方向</th><th align="center">平台</th><th align="center">延迟</th><th align="center">类型</th></tr>',
};

export default class fDepTT {
	constructor(helper) {
		this.helper = helper;
	 ...
	 *
	 * @param DepTTOptions Single configuration entry for route
	 */
	async getDepTT(DepTTOptions) {
		...
		try {
			this.helper.ReportingInfo(
				"Debug",
				"Departure Timetable",
				`Executing HAFAS search for Departure Timetable #${this.index.toString()}`,
				"fDepTT",
				"getDepTT",
				"Get",
				JSON.stringify(DepTTOptions.returnDepTTOptions()),
			);
			aDepTTResult = await this.helper.hClient.departures(
				this.StationFrom.id.toString(),
				DepTTOptions.returnDepTTOptions(),
			);
			if (aDepTTResult) {
				aDepTTResult = aDepTTResult.departures;
			}
			if (Array.isArray(aDepTTResult)) {
				if (aDepTTResult.length > this.NumDeps) {
					aDepTTResult = aDepTTResult.slice(0, this.NumDeps);
				}
			}
			...
			this.helper.ReportingInfo(
				"Debug",
				"Departure Timetable",
				// Original (volle JSON-Ausgabe):
				// `Result received for Departure Timetable #${this.index.toString()}: ${JSON.stringify(aDepTTResult)}`,
				`Result received for Departure Timetable #${this.index.toString()}: count=${Array.isArray(aDepTTResult) ? aDepTTResult.length : 0}${Array.isArray(aDepTTResult) && aDepTTResult[0] && aDepTTResult[0].line && aDepTTResult[0].line.name ? ` firstLine=${aDepTTResult[0].line.name}` : ""}`,
				"fDepTT",
				"getDepTT",
				"Get",
				//JSON.stringify(aDepTTResult),
				"suppressed-full-json",
			);
			if (this.helper.Fahrplan.config.SaveJSON !== false) {
				await this.helper.SetTextState(
					`DepartureTimetable${this.index.toString()}.JSON`,
					"Departure Timetable JSON",
					"Departure Timetable JSON",
					JSON.stringify(aDepTTResult),
					"json",
				);
			...
			if (e.hafasErrorCode === "H890") {
				this.helper.Fahrplan.log.error(
					`No departures found for Departure Timetable ${this.index} (HAFAS-Error H890)`,
				);
				this.DepTTFound = false;
			} else if (
				e.isHafasError === true &&
				e.message &&
				(e.message.startsWith("invalid/unsupported response content-type") ||
					e.message.includes("response content-type: text/html"))
			) {
				// Sonderfall: Server liefert HTML statt JSON – meist temporärer Fehler (Rate-Limit, Wartung, Blockierung oder Proxy)
				this.helper.Fahrplan.log.error(
					`HAFAS returned HTML instead of JSON for Departure Timetable ${this.index}. Likely a temporary provider/network issue (rate limit, maintenance, proxy/captive portal or blocked client). Message: ${e.message}`,
				);
				// Als Verbindungsproblem behandeln, nicht weiter eskalieren
				this.DepTTFound = false;
				// Zusätzliche Debug-Infos falls verfügbar
				if (e.url) {
					this.helper.ReportingInfo(
						"Debug",
						"Departure Timetable",
						`HTML instead of JSON details URL=${e.url} status=${e.statusCode || "n/a"} code=${e.code || "n/a"}`,
						"fDepTT",
						"getDepTT",
						"ContentType",
					);
				}
				// Versuche Body-Snippet zu loggen (nur erste 200 Zeichen, ohne Zeilenumbrüche)
				try {
					const rawBody = e.responseBody || e.body || e.html || null;
					if (rawBody && typeof rawBody === "string") {
						const snippet = rawBody.replace(/\s+/g, " ").slice(0, 200);
						this.helper.ReportingInfo(
							"Debug",
							"Departure Timetable",
							`HTML snippet: ${snippet}${rawBody.length > 200 ? "..." : ""}`,
							"fDepTT",
							"getDepTT",
							"ContentTypeSnippet",
						);
					...
					// Ignored: Snippet extraction is best-effort only
					void err; // prevent unused variable lint error
				}
			} else if (e.isHafasError === true) {
				this.helper.ReportingError(e, `HAFAS error in Departure Timetable`, "fDepTT", "getDepTT", "", {
					hafas: {
						request: e.request,
						url: e.url,
				...
				this.helper.ReportingError(
					e,
					`Exception in Departure Timetable ${this.index}`,
					"fDepTT",
					"getDepTT",
					"Get",
				);
		...
		try {
			if (this.DepTTFound === true) {
				const imaxDepartures = this.NumDeps;
				let icounterDepartures = 0;
				for (const iDeparturesCurrent in aDepTTResult) {
					...
					if (icounterDepartures >= imaxDepartures) {
						break;
					}
				}
			}
		...
		try {
			if (this.enabled === true) {
				await this.helper.SetChannel(
					`DepartureTimetable${this.index.toString()}`,
					`Departure Timetable #${this.index.toString()} - ${this.StationFrom.customname}`,
					"Departure Timetable from Adapter configuration",
				);
			}
			await this.helper.SetBoolState(
				`DepartureTimetable${this.index.toString()}.Enabled`,
				`Configuration State of Departure Timetable #${this.index.toString()}`,
	...
	async writeStates() {
		try {
			for (const iDeparturesCurrent in this.Departures) {
				this.Departures[iDeparturesCurrent].write(
					`DepartureTimetable${this.index}.${iDeparturesCurrent}`,
					this.index,
					iDeparturesCurrent,
				);
			}
	...
	async writeHTML() {
		try {
			...
			await this.helper.SetTextState(
				`DepartureTimetable${this.index.toString()}.HTML`,
				"HTML",
				"HTML",
				this.html,
				"html",
			...
			this.helper.ReportingError(e, `Exception in Departure Timetable ${this.index}`, "fDepTT", "writeHTML", "", {
				json: this.json,
			});
			throw this.helper.ErrorCustom("HANDLED");
		}
