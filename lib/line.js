export default class fLine {
	 ...
	 *
	 * @param Section Section object (contains direction)
	 */
	async setLine(Section) {
		try {
			...
			if (Section.line.operator) {
				this.operator = Section.line.operator.name;
			}
			this.product = Section.line.product;
			this.platform = Section.platform;
			this.plannedplatform = Section.plannedPlatform;
		} catch (e) {
	 ...
	 *
	 * @param BasePath Path to channel with station information
	 */
	async writeLine(BasePath) {
		try {
			...
			await this.helper.SetTextState(`${BasePath}.Platform`, "Line Platform", "Line Platform", this.platform);
			await this.helper.SetTextState(
				`${BasePath}.PlannedPlatform`,
				"Line PlannedPlatform",
				"Line PlannedPlatform",
				this.plannedplatform,
			);
		} catch (e) {
	...
	translateProduct() {
		try {
			if (this.helper.hProfile !== null && this.product !== undefined && this.product !== "") {
				const result = this.helper.hProfile.products.filter(obj => {
					return obj.id === this.product;
				});
				// Prüfen ob ein Produkt gefunden wurde
				if (result.length > 0 && result[0].short) {
					return result[0].short;
				}
			}
			// Fallback: Name der Linie falls vorhanden, sonst product-Code
			return this.name || this.product || "?";
		} catch (e) {
			this.helper.ReportingError(e, `Exception in Line`, "fLine", "translateProduct", "", {
