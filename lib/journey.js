...
import fSection from "./section.js";

// Language specific Header for HTML journey output
const sJourneyHTMLHeader = {
	en: '<tr><th align="left">Station</th><th align="center">Time</th><th align="center">Platform</th><th align="center">Delay</th><th align="center">Line</th></tr>',
	de: '<tr><th align="left">Station</th><th align="center">Zeit</th><th align="center">Plattform</th><th align="center">Verspätung</th><th align="center">Line</th></tr>',
	ru: '<tr><th align="left">остановка</th><th align="center">Время</th><th align="center">Платформа</th><th align="center">задержка</th><th align="center">Line</th></tr>',
	pt: '<tr><th align="left">estação</th><th align="center">Tempo</th><th align="center">Plataforma</th><th align="center">Demora</th><th align="center">Line</th></tr>',
	nl: '<tr><th align="left">Station</th><th align="center">Tijd</th><th align="center">Platform</th><th align="center">Vertraging</th><th align="center">Line</th></tr>',
	fr: '<tr><th align="left">Station</th><th align="center">Temps</th><th align="center">Plate-forme</th><th align="center">Retard</th><th align="center">Line</th></tr>',
	it: '<tr><th align="left">Stazione</th><th align="center">Tempo</th><th align="center">piattaforma</th><th align="center">Ritardo</th><th align="center">Line</th></tr>',
	es: '<tr><th align="left">Estación</th><th align="center">Hora</th><th align="center">Plataforma</th><th align="center">Retrasar</th><th align="center">Line</th></tr>',
	pl: '<tr><th align="left">Stacja</th><th align="center">Czas</th><th align="center">Platforma</th><th align="center">Opóźnienie</th><th align="center">Line</th></tr>',
	"zh-cn":
		'<tr><th align="left">站</th><th align="center">时间</th><th align="center">平台</th><th align="center">延迟</th><th align="center">线路</th></tr>',
};

export default class fJourney {
	constructor(helper) {
		this.helper = helper;
		this.Sections = [];
	 ...
	 *
	 * @param Journey JSON result from HAFAS
	 */
		...
		try {
			...
			for (const iSectionCurrent in Journey.legs) {
				...
				if (aConnSub["departurePlatform"]) {
					CurrSection.StationFrom.platform = aConnSub.departurePlatform;
				}
				if (aConnSub["plannedDeparturePlatform"]) {
					CurrSection.StationFrom.platformplanned = aConnSub.plannedDeparturePlatform;
				}
				if (aConnSub["arrivalPlatform"]) {
					CurrSection.StationTo.platform = aConnSub.arrivalPlatform;
				}
				if (aConnSub["plannedArrivalPlatform"]) {
					CurrSection.StationTo.platformplanned = aConnSub.plannedArrivalPlatform;
				}
				...
				if (aConnSub.departureDelay !== null && aConnSub.departureDelay >= 0) {
					...
					if (
						aConnSub.departureDelay === 0 &&
						aConnSub.departureDelay < this.helper.Fahrplan.config.DelayTime * 60
					) {
				...
				if (aConnSub.arrivalDelay !== null && aConnSub.arrivalDelay >= 0) {
					...
					if (
						aConnSub.arrivalDelay === 0 &&
						aConnSub.arrivalDelay < this.helper.Fahrplan.config.DelayTime * 60
					) {
				...
				if (aConnSub["line"]) {
					...
					this.line = CurrSection.Line.name;
				}
				// Reachability
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "parseJourney", "", { json: Journey });
	 ...
	 *
	 * @param BasePath Path to channel with journey information
	 * @param RouteIndex Index number of Route
	 */
	async checkDelay(BasePath, RouteIndex) {
		try {
			const ConfigDelays = this.helper.Fahrplan.config.delays || [];
			const RouteDelays = ConfigDelays.filter(
				delay => delay.route === RouteIndex.toString() && delay.enabled === true && delay.notistart !== "",
			);
			if (RouteDelays.length > 0) {
				this.helper.ReportingInfo(
					"Debug",
					"Route",
					`Configured Delays for Route #${RouteIndex}: ${JSON.stringify(RouteDelays)}`,
					"fJourney",
					"checkDelay",
					"",
					JSON.stringify(RouteDelays),
				);
			}
			for (const iRouteDelayCurrent in RouteDelays) {
				const RouteDelay = RouteDelays[iRouteDelayCurrent];
				if (
					(RouteDelay.departplan === "" ||
						RouteDelay.departplan === this.helper.Fahrplan.formatDate(new Date(this.departure), "hh:mm")) &&
					(RouteDelay.days.includes("7") ||
						RouteDelay.days.includes(new Date(this.departure).getDay().toString()))
				) {
					...
					if (new Date() >= new Date(NotiStartTime) && new Date() <= new Date(this.departure)) {
						...
						if (
							this.departureDelaySeconds !== 0 &&
							this.departureDelaySeconds >= this.helper.Fahrplan.config.DelayTime * 60
						) {
							const OldNotifyValue = await this.helper.Fahrplan.getStateAsync(`${BasePath}.NotifyValue`);
							//this.helper.ReportingInfo("Debug", "Route", `OldNotifyValue: ${JSON.stringify(OldNotifyValue)}`, "fJourney", "checkDelay", "", JSON.stringify(OldNotifyValue));
							if (
								OldNotifyValue === null ||
								!OldNotifyValue ||
								(OldNotifyValue &&
									OldNotifyValue.val !== null &&
									parseInt(this.departureDelaySeconds.toString()) !==
										parseInt(OldNotifyValue.val.toString()))
							) {
								this.notify = true;
								this.notifyValue = this.departureDelaySeconds;
								this.notifyText = await this.buildDelayNotification();
								//this.helper.ReportingInfo("Debug", "Route", `DELAY NOTIFICATION: ${this.notifyText}`, "fJourney", "checkDelay");
								if (RouteDelay.output_id !== "") {
									// Buidling output string
									await this.helper.Fahrplan.setForeignStateAsync(
										RouteDelay.output_id,
										this.notifyText,
									);
								}
							}
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "checkDelay", "", { json: this.json });
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	async buildDelayNotification() {
		try {
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "buildDelayNotification", "", {
				json: this.json,
			});
			throw this.helper.ErrorCustom("HANDLED");
		}
	 ...
	 * @param BasePath Path to channel with journey information
	 * @param RouteIndex Index number of Route
	 * @param JourneyIndex Index number of Journey
	 */
	async writeJourney(BasePath, RouteIndex, JourneyIndex) {
		try {
			await this.helper.SetChannel(
				`${BasePath}`,
				`Connection ${this.StationFrom.name} - ${this.StationTo.name}`,
				`Connection`,
			);
			...
			if (this.helper.Fahrplan.config.SaveObjects >= 1) {
				...
				await this.helper.SetBoolState(
					`${BasePath}.TransfersReachable`,
					"TransfersReachable",
					"TransfersReachable",
					this.transfersReachable,
				);
				...
				await this.helper.SetNumState(
					...
					"Arrival",
					"Arrival",
					new Date(this.arrival).getTime(),
					"date",
				);
				await this.helper.SetNumState(
					`${BasePath}.ArrivalPlanned`,
					"ArrivalPlanned",
					"ArrivalPlanned",
					new Date(this.arrivalPlanned).getTime(),
					"date",
				);
				await this.helper.SetNumState(
					`${BasePath}.ArrivalDelaySeconds`,
					"ArrivalDelaySeconds",
					"ArrivalDelaySeconds",
					this.arrivalDelaySeconds,
				);
				await this.helper.SetBoolState(
					`${BasePath}.ArrivalOnTime`,
					"ArrivalOnTime",
					"ArrivalOnTime",
					this.arrivalOnTime,
				);
				await this.helper.SetBoolState(
					`${BasePath}.ArrivalDelayed`,
					"ArrivalDelayed",
					"ArrivalDelayed",
					this.arrivalDelayed,
				);
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "writeJourney", "", { json: this.json });
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	createHTML() {
		try {
			...
			if (this.arrivalOnTime === true) {
				sHTML = `${sHTML}<td><font color="${ColorOntime}">${this.helper.Fahrplan.formatDate(new Date(this.arrival), "hh:mm")}</font></td>`;
			} else if (this.arrivalDelayed === true) {
				sHTML = `${sHTML}<td><font color="${ColorDelay}">${this.helper.Fahrplan.formatDate(new Date(this.arrival), "hh:mm")}</font></td>`;
			} else {
			...
			if (this.helper.Fahrplan.config.CreateHTML >= 3) {
				if (this.StationTo.platform === this.StationTo.platformplanned) {
					sHTML = `${sHTML}<td><font color="${ColorOntime}">${this.StationTo.platform}</font></td>`;
				} else {
					sHTML = `${sHTML}<td><font color="${ColorDelay}">${this.StationTo.platform}</font></td>`;
				}
			}
			// departure Line
			if (this.helper.Fahrplan.config.CreateHTML >= 4) {
				sHTML = `${sHTML}<td>${this.line}</td>`;
			}
			sHTML = `${sHTML}</tr>`;
			return sHTML;
		} catch (e) {
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "createHTML", "", { json: this.json });
			throw this.helper.ErrorCustom("HANDLED");
		}
	}
	...
	async writeJourneyHTML(BasePath) {
		try {
			...
			if (this.helper.Fahrplan.config.CreateHTMLJourney >= 1) {
				...
				for (const iSectionsCurrent in this.Sections) {
					...
					if (oSection.departureOnTime === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${this.helper.Fahrplan.formatDate(new Date(oSection.departure), "hh:mm")}</font></td>`;
					} else if (oSection.departureDelayed === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${this.helper.Fahrplan.formatDate(new Date(oSection.departure), "hh:mm")}</font></td>`;
					} else {
					...
					if (oSection.StationFrom.platform === null) {
						...
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${oSection.StationFrom.platform}</font></td>`;
					} else {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${oSection.StationFrom.platform}</font></td>`;
					}
					// Delay
					const dDelay = Math.ceil(oSection.departureDelaySeconds / 60);
					if (oSection.departureOnTime === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${dDelay}</font></td>`;
					} else if (this.departureDelayed === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${dDelay}</font></td>`;
					} else {
						this.html = `${this.html}<td align="center">${dDelay}</td>`;
					}
					// Line
					if (oSection.Line.name !== null) {
						this.html = `${this.html}<td>${oSection.Line.name}</td>`;
					} else {
						this.html = `${this.html}<td>-</td>`;
					}
					// Arrival
					...
					if (oSection.arrivalOnTime === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${this.helper.Fahrplan.formatDate(new Date(oSection.arrival), "hh:mm")}</font></td>`;
					} else if (oSection.arrivalDelayed === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${this.helper.Fahrplan.formatDate(new Date(oSection.arrival), "hh:mm")}</font></td>`;
					} else {
					...
					if (oSection.StationTo.platform === null) {
						...
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${oSection.StationTo.platform}</font></td>`;
					} else {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${oSection.StationTo.platform}</font></td>`;
					}
					// Delay
					const aDelay = Math.ceil(oSection.arrivalDelaySeconds / 60);
					if (oSection.arrivalOnTime === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorOntime}">${aDelay}</font></td>`;
					} else if (this.arrivalDelayed === true) {
						this.html = `${this.html}<td align="center"><font color="${ColorDelay}">${aDelay}</font></td>`;
					} else {
			...
			this.helper.ReportingError(e, `Exception in Journey`, "fJourney", "writeJourneyHTML", "", {
				json: this.json,
			});
			throw this.helper.ErrorCustom("HANDLED");
		}
