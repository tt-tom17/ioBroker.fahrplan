	...
	async getRoute(RouteOptions) {
		...
		try {
			...
			this.helper.ReportingInfo(
				...
				`Result received for Route #${this.index.toString()}: journeys=${aRouteResult && aRouteResult.journeys ? aRouteResult.journeys.length : 0}${aRouteResult && aRouteResult.journeys && aRouteResult.journeys[0] && aRouteResult.journeys[0].legs ? ` firstLegs=${aRouteResult.journeys[0].legs.length}` : ""}`,
				"fRoute",
				"getRoute",
				"RouteSearch",
			...
			if (e.hafasErrorCode === "H890") {
				...
				// Server liefert HTML (Fehlerseite, Rate-Limit, Wartung, Proxy) – als temporäres Verbindungsproblem behandeln
				this.helper.Fahrplan.log.error(
					`HAFAS returned HTML instead of JSON for Route ${this.index}. Likely temporary provider/network issue (rate limit, maintenance, proxy/captive portal or blocked client). Message: ${e.message}`,
				);
				this.JourneysFound = false;
				if (e.url) {
					this.helper.ReportingInfo(
			...
			if (this.JourneysFound === true) {
				for (const iJourneysCurrent in aRouteResult.journeys) {
					if (parseInt(iJourneysCurrent) < this.NumDeps) {
						...
						// Original (volle Journey JSON Ausgabe):
						// this.helper.ReportingInfo(
						// 	"Debug",
						// 	"Route",
						...
						let legsCount = 0;
						try {
							legsCount = Array.isArray(aConn.legs) ? aConn.legs.length : 0;
						} catch (err) {
							void err;
						}
						const firstLeg = aConn && aConn.legs && aConn.legs[0] ? aConn.legs[0] : null;
						const firstMode =
							firstLeg && firstLeg.line && firstLeg.line.name
								? firstLeg.line.name
								: firstLeg && firstLeg.mode
									? firstLeg.mode
									: "n/a";
						this.helper.ReportingInfo(
							"Debug",
							"Route",
							`Route #${this.index.toString()} Journey #${iJourneysCurrent}: legs=${legsCount} firstMode=${firstMode}`,
							"fRoute",
							"getRoute",
							"Iteration",
							"suppressed-full-journey-json",
						);
