...
export default class fOptions {
	 ...
	 *
	 * @param Products Configured Products in Configuration of Adapter
	 */
	...
	returnRouteOptions() {
		try {
			...
			if (this.depsOffsetMin > 0) {
				aRouteOptions["departure"] = new Date(new Date().getTime() + this.depsOffsetMin * 60000);
			}
			if (this.via !== "") {
				aRouteOptions["via"] = this.via;
			}
			if (this.transfers >= 0) {
				aRouteOptions["transfers"] = this.transfers;
			}
			if (this.bycicles === true) {
				aRouteOptions["bike"] = this.bycicles;
			}
			if (this.products !== null) {
				aRouteOptions["products"] = this.products;
			}
	...
	returnDepTTOptions() {
		try {
			const aDepTTOptions = {
				...
				when: new Date(new Date().getTime() + this.depsOffsetMin * 60000),
				duration: 120 + this.depsOffsetMin,
			};
			if (this.products !== null) {
				aDepTTOptions["products"] = this.products;
			}
...
/*
export function returnDepTTOptions() {
